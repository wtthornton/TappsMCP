...
When the **TappsMCP** MCP server is configured in your host, you can use its tools for code quality, doc lookup, and domain expert advice. The steps below are **recommended**; consider following them to avoid hallucinated APIs and missed quality steps. You may skip or defer steps when appropriate for your task.
