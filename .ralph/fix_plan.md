...
- [x] 92.5: Execute story — docs/epics/stories/STORY-92.5-improved-gherkin-scaffolding.md ✓ Added `_derive_given()`, `_derive_when()`, `_derive_then()` static methods to StoryGenerator. Updated `_render_gherkin_criteria` to use derivation methods with bracket placeholder fallback. `_derive_given` uses role; `_derive_when` strips "to " prefix from want or falls back to AC verb phrase; `_derive_then` appends " successfully" to AC text. Updated `test_gherkin_acceptance_criteria` test. Added 21 unit tests in TestImprovedGherkinScaffolding.
