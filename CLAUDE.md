...
- **`python3 -c` is blocked AND cascades (TAP-2436)**: `.ralph/hooks/validate-command.sh` unconditionally blocks `python3 -c` (and `uv run python -c`). This is a hard block — exit 2. **Critical side-effect:** the block also CANCELS every other tool call in the same parallel Bash group. If `python3 -c` is batched with `python3 /tmp/snippet.py`, the script-file call is silently cancelled too. Always write snippets to `/tmp/snippet.py` first (via Write), then issue `python3 /tmp/snippet.py` in its OWN isolated Bash call — never batched alongside a `python3 -c` call. **Heredoc bypass is also blocked (TAP-2465):** `python3 - << 'EOF'` and `python3 /dev/stdin` are semantically equivalent to `python3 -c` — they all execute arbitrary inline Python. Do NOT attempt these as a workaround; they carry the same cascade risk.
...
- **Dev-repo MCP uses global CLIs, not `uv run` source** (Epic 109): Cursor loads `nlt-code-quality` + `nlt-platform-admin` via `.cursor/bin/*-serve.sh`, which `exec` the global `tapps-mcp serve --profile nlt-*` binaries. `.cursor/mcp.json` is gitignored (local env). After changing `packages/tapps-mcp` or `packages/docs-mcp`, reinstall globals and reload MCP: `uv tool install --reinstall --from packages/tapps-mcp tapps-mcp && uv tool install --reinstall --from packages/docs-mcp docs-mcp`. Regenerate config: `tapps-mcp init --host cursor --force --allow-package-init --no-uv`.
...
<!-- BEGIN: tapps-obligations v3.12.19 -->
