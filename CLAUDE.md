...
TappsMCP is an **MCP server** providing deterministic code quality tools to LLMs and AI coding assistants. It scores Python files, runs security scans, enforces quality gates, looks up library docs, and validates configs -- all via structured MCP tool calls (34 tools). Any MCP-capable client (Claude Code, Cursor, VS Code Copilot) can use it. If you are a consuming project, see [AGENTS.md](AGENTS.md) instead.
...
| **tapps-mcp** | `packages/tapps-mcp/` | Code quality MCP server (34 tools) |
...
<!-- BEGIN: tapps-obligations v3.12.39 -->
