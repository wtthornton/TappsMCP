...
- **Dev-repo MCP runs the deployed global CLI, NOT live `.venv`/`uv run` source** (reaffirmed; supersedes the Epic 116 `uv run` attempt): Cursor loads the six `nlt-*` servers via `.cursor/bin/nlt-*-serve.sh`, which `exec` the global `~/.local/bin/{tapps-mcp,tapps-platform,docsmcp} serve --profile nlt-*` binaries (the isolated `uv tool install` env). **Why not `.venv`/`uv run`:** the dev repo constantly runs pytest/`uv` against its workspace `.venv`; launching the MCP fleet from that same env let test/build churn crash live servers, so Cursor flapped error↔good on a 5-min relaunch backoff. The global tool env is isolated and stays up. **Source changes go live only on an explicit deploy — never automatically:** `uv tool install --reinstall --from packages/tapps-mcp tapps-mcp && uv tool install --reinstall --from packages/docs-mcp docs-mcp`, then reload MCP. `.cursor/mcp.json` is gitignored (local env); regenerate wrappers/config with `tapps-mcp init --host cursor --force --allow-package-init --no-uv` (the generator emits the global form for the dev monorepo via `_resolve_dev_monorepo_launch`).
