...
- **`python3 -c` is blocked AND cascades (TAP-2436)**: `.ralph/hooks/validate-command.sh` unconditionally blocks `python3 -c` (and `uv run python -c`). This is a hard block — exit 2. **Critical side-effect:** the block also CANCELS every other tool call in the same parallel Bash group. If `python3 -c` is batched with `python3 /tmp/snippet.py`, the script-file call is silently cancelled too. Always write snippets to `/tmp/snippet.py` first (via Write), then issue `python3 /tmp/snippet.py` in its OWN isolated Bash call — never batched alongside a `python3 -c` call.
- **Linear multi-issue reads require the `linear-read` skill**: Calling `mcp__plugin_linear_linear__list_issues` directly (without a prior `tapps_linear_snapshot_get`) triggers the cache-gate hook. Use the `linear-read` skill for all list-style reads; it runs the `snapshot_get → list_issues → snapshot_put` dance and uses `projection="compact"` (~5 kB for 50 issues) to avoid the 25 k-token inline ceiling that causes `python3 -c` parse cascades.
