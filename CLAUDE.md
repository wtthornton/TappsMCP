...
- **tapps-brain version**: TappsMCP pins tapps-brain at `v1.1.0` (git tag). Profile/promotion modules (`tapps_brain.profile`, `tapps_brain.promotion`) are available. Imports still use `try/except ImportError` for defensive degradation in non-standard installs.
