...
TappsMCP is an **MCP server** providing deterministic code quality tools to LLMs and AI coding assistants. It scores Python files, runs security scans, enforces quality gates, looks up library docs, and validates configs -- all via structured MCP tool calls (48 tools). Any MCP-capable client (Claude Code, Cursor, VS Code Copilot) can use it. If you are a consuming project, see [AGENTS.md](AGENTS.md) instead.
...
| **tapps-mcp** | `packages/tapps-mcp/` | Code quality MCP server (48 tools) |
...
- **Linear multi-issue reads require the `linear-read` skill (TAP-2466)**: Calling `mcp__plugin_linear_linear__list_issues` directly (without a prior `tapps_linear_snapshot_get`) triggers the cache-gate hook. Use the `linear-read` skill for all list-style reads; it runs the `snapshot_get → list_issues → snapshot_put` dance. **Always pass `projection="compact"` to `tapps_linear_snapshot_get`** — compact mode returns ~5 kB for 50 issues; full mode returns ~80 kB (>25k tokens) which overflows the Read tool ceiling and forces a fallback parse. Note: `list_issues` file output uses `id`/`status`/`statusType` field names, NOT `identifier`/`state.name`/`state.type` — parse accordingly.
...
- **tapps-brain version pin**: the floor is `>=3.28.0,<4` per [ADR-0033](docs/adr/0033-pin-tapps-brain-version-floor-at-3280.md) (supersedes [ADR-0013](docs/adr/0013-pin-tapps-brain-version-floor-at-3240.md)) — 3.28.0 is the release adding `web_research` / `research_fetch`, which `BrainBridge` binds for `tapps_research`. The workspace source pin tracks tag `v3.28.1` per [ADR-0011](docs/adr/0011-pin-tapps-brain-by-tag.md). Transport selection is documented in [ADR-0001](docs/adr/0001-in-process-agentbrain-via-brainbridge.md) (in-process default; HTTP when `memory.brain_http_url` is set). Consumer brain wiring: [docs/operations/CONSUMER-REPO-BRAIN-WIRING.md](docs/operations/CONSUMER-REPO-BRAIN-WIRING.md).
...
- **Dev-repo MCP runs blue/green deployed CLIs, NOT live `.venv`/`uv run` source** (reaffirmed; supersedes the Epic 116 `uv run` attempt and in-place `uv tool install --reinstall`): Cursor loads the six `nlt-*` servers via `.cursor/bin/nlt-*-serve.sh`, which prefer `~/.tapps-mcp/current/bin/{tapps-mcp,tapps-platform,docsmcp}` when present (immutable release venv), else fall back to legacy `~/.local/bin/*` shims. **Why not `.venv`/`uv run`:** the dev repo constantly runs pytest/`uv` against its workspace `.venv`; launching the MCP fleet from that same env let test/build churn crash live servers, so Cursor flapped error↔good on a 5-min relaunch backoff. **Deploy contract:** `tapps-mcp deploy-local` (or `upgrade-fleet --reinstall-clis`) builds a new release under `~/.tapps-mcp/releases/<version>-<sha>/`, smoke-tests it, atomically flips `~/.tapps-mcp/current`, and GCs old releases — running servers stay pinned to their release dir; reload MCP in Cursor for new launches to pick up the build. Never use in-place `uv tool install --reinstall` on the dev monorepo while agents are live. `.cursor/mcp.json` is gitignored (local env); regenerate wrappers/config with `tapps-mcp init --host cursor --force --allow-package-init --no-uv`.
...
- **Skill template conventions** (description format, no `disable-model-invocation` on generated skills, progressive-disclosure threshold) are documented in [docs/SKILL_AUTHORING.md](docs/SKILL_AUTHORING.md). Follow these rules when adding or editing a skill: body text **and its frontmatter** (`description:`, `allowed-tools:`, etc.) live in `pipeline/assets/claude_skills/<name>.md`; `pipeline/platform_skills.py` holds only the one-line `CLAUDE_SKILLS[...] = _load_claude_skill(...)` dict wiring, no frontmatter. Three domain skills have no asset file and are edited in `pipeline/platform_domain_skills.py` instead. See SKILL_AUTHORING.md's "Where a skill actually lives" section — the same split applies to CLAUDE_AGENTS and the docs-automation dicts.
...
<!-- BEGIN: tapps-obligations v3.12.86 -->
