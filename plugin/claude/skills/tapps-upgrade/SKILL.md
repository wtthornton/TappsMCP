...
allowed-tools: Bash mcp__plugin_tapps-mcp_tapps-mcp__tapps_session_start mcp__plugin_tapps-mcp_tapps-mcp__tapps_doctor mcp__plugin_tapps-mcp_tapps-mcp__tapps_checklist
...
3. **Verify new version is live.** Call `mcp__plugin_tapps-mcp_tapps-mcp__tapps_session_start(quick=false, force=true)`. Confirm `server.version` matches target and `diagnostics.install_drift.drift_detected == false`. If drift persists, the server wasn't restarted — go back to step 2.
...
6. **Verify.** Run `tapps-mcp doctor` AND `mcp__plugin_tapps-mcp_tapps-mcp__tapps_checklist(task_type="upgrade")`. Surface any problems — do not declare done on a failure. Doctor NLT row shows eager (Claude) vs listed (Cursor).
