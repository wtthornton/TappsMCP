...
2. **Verify the checklist.** Call `mcp__plugin_tapps-mcp_tapps-mcp__tapps_checklist(task_type=<feature|bugfix|refactor|security|review>)`. Read the inline **`usage_gaps`** block — not only `complete` / `missing_steps`. If `complete: false`, address each entry in `missing_steps` and re-run.
...
3. **Clear doc-lookup gaps.** When `usage_gaps.gaps` includes `lookup_docs_underused`,
   ...
   - Call `mcp__plugin_tapps-mcp_tapps-mcp__tapps_lookup_docs(library=<name>, topic=<relevant-api>)` for **each** listed library (retrospective MCP lookups clear telemetry gaps; cache hits are fine — ADR-0021).
   - CLI `tapps-mcp lookup-docs` also records `.lookup-docs-events.jsonl` for the next session.
   - Re-run `mcp__plugin_tapps-mcp_tapps-mcp__tapps_checklist` until `usage_gaps.gaps` is empty **and** `complete: true`.
...
3b. **Creator-verifier (feature/review).** Before declaring done on feature or review work,
   ...
   Re-run `mcp__plugin_tapps-mcp_tapps-mcp__tapps_checklist` until `contract_assertions_unverified` and `creator_verifier_skipped`
