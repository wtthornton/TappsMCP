...
from tapps_mcp.memory.models import MemoryEntry, MemoryScope, MemoryTier
from tapps_mcp.memory.store import MemoryStore, _MAX_ENTRIES
...
class TestMemoryStoreRAGSafety:
    ...
    def test_blocked_content_returns_error(self, store: MemoryStore) -> None:
        ...
        with patch(
            "tapps_mcp.memory.store.check_content_safety"
        ) as mock_safety:
...
class TestMemoryStoreSingleton:
    """Tests for singleton pattern in server_helpers."""
    ...
    def test_get_and_reset(self, tmp_path: Path) -> None:
        ...
        # Just verify the reset function exists and is callable
        _reset_memory_store_cache()
