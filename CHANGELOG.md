...
- **Sync hooks to external loop templates** ([TAP-2494](https://linear.app/tappscodingagents/issue/TAP-2494)/[TAP-2501](https://linear.app/tappscodingagents/issue/TAP-2501)/[TAP-2502](https://linear.app/tappscodingagents/issue/TAP-2502)).
...
- **`feat(eval): tool-description eval harness under scripts/eval-descriptions/`.** Greenfield harness that measures tool-selection accuracy of the live tapps-mcp MCP catalog via `claude -p --output-format=stream-json --strict-mcp-config` — Claude Code OAuth handles auth (no `ANTHROPIC_API_KEY` plumbing), Max plan covers cost. Each scenario in [`scenarios.yaml`](scripts/eval-descriptions/scenarios.yaml) (26 scenarios across 11 categories) is a user-intent prompt; the harness captures the first MCP `tool_use` event and scores against `expected_tool` + `acceptable_alternatives`. [`compare.py`](scripts/eval-descriptions/compare.py) A/B's two git refs via `git worktree` + `uv sync --all-packages` per ref. [`report.py`](scripts/eval-descriptions/report.py) emits a Markdown report with raw + noise-adjusted numbers (the latter excludes scenarios that errored on either side — typically 120s MCP cold-start timeouts that are orthogonal to description quality). 26 unit tests in [`test_eval_descriptions.py`](packages/tapps-mcp/tests/unit/test_eval_descriptions.py) cover the pure-logic functions (parsing, scoring, diffing); the Claude-CLI flow is exercised by manual runs, not unit tests.
  ...
  **First baseline run** (`cc1d340^` pre-description-rewrite vs `HEAD` post-rewrite) is committed at [`docs/benchmarks/2026-05-19-description-eval.md`](docs/benchmarks/2026-05-19-description-eval.md). **Headline:** noise-adjusted, the rewrite produced 1 true regression (`release_update_announce`: baseline=exact → HEAD=no_tool), 0 true improvements, and 1 stable failure across both versions (`decompose_vague_task`). The raw numbers show -8.3pt because 8 scenarios timed out at 120s (4 in each run — different scenarios per side, no overlap — strong signal that they are infrastructure flakes, not description regressions). This is exactly the value the eval harness was built to surface — the rewrite would have shipped without measurement otherwise. Next-pass targets: fix `release_update_announce` description trigger, fix `decompose_vague_task` (stable failure on both versions), and tune the 120s timeout up / pre-warm MCP servers to eliminate cold-start noise.

- **`feat(mcp): rewrite all tool descriptions in 2026 MCP / Context7 pattern` (v3.10.16, commit `cc1d340`).** Applies a 2026-validated tool-description pattern to every public MCP handler in `tapps-mcp` (~30 tools across `server.py`, `server_pipeline_tools.py`, `server_scoring_tools.py`, `server_analysis_tools.py`, `server_memory_tools.py`, `server_linear_tools.py`, `server_metrics_tools.py`, `server_release_tools.py`, `tools/decompose_helpers.py`, `tools/validate_changed.py`). New server-level `instructions` field on the `FastMCP("TappsMCP", instructions=...)` instance carries the cross-tool obligations once — per-tool descriptions no longer repeat blanket `MUST`/`REQUIRED` markers (the empirical "useless qualifiers" smell per arXiv 2602.18914, n=10,831 servers). Each tool description leads with a declarative one-line "what it does", followed by a "when to call" trigger clause and a "when NOT to call" sibling-tool disambiguation; parameter descriptions in the `Args:` block (which FastMCP extracts into `inputSchema.properties[*].description` per SEP-1382) carry good/bad examples and format hints. The overlapping scoring family (`tapps_quick_check` vs `tapps_score_file` vs `tapps_quality_gate` vs `tapps_validate_changed`) is given explicit cross-references so the LLM has a clean decision boundary — the "repeated tool name" smell hits 73% of MCP servers per the eval study. Pattern sources: [Anthropic "Writing tools for agents"](https://www.anthropic.com/engineering/writing-tools-for-agents) (Opus 4.5 selection accuracy 79.5% → 88.1% with this shape), MCP spec 2025-11-25, [arXiv 2602.14878](https://arxiv.org/html/2602.14878v1) + [arXiv 2602.18914](https://arxiv.org/html/2602.18914), [SEP-1382](https://github.com/modelcontextprotocol/modelcontextprotocol/issues/1382), Context7 MCP source, and github-mcp-server as a production reference. 4692 unit tests pass; quality gate passes on all 10 modified files (scores 85–100). Follow-up: declare `outputSchema` on the 8 high-traffic tools (`tapps_session_start`, `tapps_quick_check`, `tapps_validate_changed`, `tapps_score_file`, `tapps_lookup_docs`, `tapps_memory`, `tapps_doctor`, `tapps_checklist`).
- **`feat(mcp-config): wire TAPPS_MCP_CONTEXT7_API_KEY into default .mcp.json env block` (v3.10.15, commit `b7f0ba4`).** `_build_server_entry` in [`distribution/setup_generator.py:288-296`](packages/tapps-mcp/src/tapps_mcp/distribution/setup_generator.py#L288-L296) now writes `TAPPS_MCP_CONTEXT7_API_KEY: "${TAPPS_MCP_CONTEXT7_API_KEY}"` into every generated `.mcp.json` (Claude Code, Cursor, VS Code) alongside the existing brain env keys. Consumers who export the var in their shell get Context7-backed docs lookup automatically; ones who don't fall back to llms.txt as before. Until now, only the `--with-context7` CLI flag wired the key, and the MCP-tool form of `tapps_init` had no Context7 parameter — so agent-driven init shipped a `.mcp.json` that never propagated the var into the server process, and most consumers were silently degrading on every `tapps_lookup_docs` call. Upgrade picks it up automatically: `_merge_config` does `{**old_env, **new_env}` with `upgrade_mode=True`, so existing consumer configs get the key added on next `tapps_upgrade`. Literal/plaintext values get normalized to `${VAR}` interpolation for commit safety (test: `test_upgrade_normalizes_literal_context7_key_to_substitution`). Truly user-managed env keys (like `OPENAI_API_KEY`) survive unchanged.
- **`feat(diagnostics): install-drift detection in tapps_session_start + tapps doctor` ([TAP-2129](https://linear.app/tappscodingagents/issue/TAP-2129)).** New `InstallDriftDiagnostic` ([common/models.py](packages/tapps-core/src/tapps_core/common/models.py)) surfaced through `tapps_session_start` ([diagnostics.py](packages/tapps-mcp/src/tapps_mcp/diagnostics.py)). On each session start, compares the running process's `tapps_mcp.__version__` and `docs_mcp.__version__` against the versions reported by the global `~/.local/bin/tapps-mcp` and `~/.local/bin/docsmcp` binaries. When drift is detected, the payload's `diagnostics.install_drift.remediation_hint` carries the literal `uv tool install -e --reinstall <path>` command for copy-paste. Silently skipped when no global binary is present (dev running purely from project venv). Never raises. `tapps doctor` parity: existing `check_binary_version_mismatch` refactored through a shared helper; new `check_docsmcp_binary_version_mismatch` added with the same modern remediation hint. Test coverage: [`test_install_drift.py`](packages/tapps-mcp/tests/unit/test_install_drift.py) — 9 tests across matched / drifted / no-global branches.
...
- **`feat(brain-bridge): remove ProfileMismatchError preflight; let the wire gate` ([TAP-2100](https://linear.app/tappscodingagents/issue/TAP-2100)).** `HttpBrainBridge._do_mcp_post` no longer short-circuits on `_exposed_tools` ([brain_bridge.py:1484](packages/tapps-core/src/tapps_core/brain_bridge.py#L1484)). Under tapps-brain v3.19.0+ the `full` / `operator` profiles default to an 8-tool eager `tools/list` ([TAP-1985](https://linear.app/tappscodingagents/issue/TAP-1985), deferred-loading) — the remaining 51 tools are still callable via `tools/call` but absent from the catalog. The preflight produced 22 false rejections out of 27 `_BRIDGE_USED_TOOLS` under the new shape (entire memory CRUD surface, Hive surface, feedback flywheel, session memory). The wire is now authoritative for the gating decision; a genuinely gated tool still surfaces as `ToolNotInProfileError` from the brain's `-32602 INVALID_PARAMS` response (one round-trip later, with `data.suggested_profile` when available per [TAP-2096](https://linear.app/tappscodingagents/issue/TAP-2096) + [TAP-1972](https://linear.app/tappscodingagents/issue/TAP-1972)). `_negotiate_profile_locked` still populates `_exposed_tools` for diagnostics (`profile_status()`, `tapps doctor check_brain_profile`). The `ProfileMismatchError` class is preserved for back-compat (no current raise site) — callers that `except ProfileMismatchError` keep compiling but will only catch `ToolNotInProfileError` going forward.
...
- **`feat(brain-bridge): tools/list negotiation + typed ProfileMismatchError` ([TAP-1629](https://linear.app/tappscodingagents/issue/TAP-1629)).** `HttpBrainBridge` now runs a `tools/list` + `profile_info` handshake after `initialize` and caches the result. `profile_status()` returns the active server profile, the exposed-tool set, and any `_BRIDGE_USED_TOOLS` missing from it (`memory_save`, `memory_get`, `memory_delete`, `memory_search`, `memory_list`, `memory_recall`, `memory_reinforce`, `memory_supersede`, `hive_search`, `hive_status`, `hive_propagate`, `agent_register`). Calls into a gated bridge tool short-circuit with a typed `ProfileMismatchError` (subclass of `ToolNotInProfileError`, carrying `data.transport=client_preflight`) before reaching the wire. `tapps_memory(action=health)` includes a `brain_profile` block; `tapps doctor` adds a `tapps-brain capability profile` row probing tools/list synchronously.
- **`feat(memory): knowledge graph actions — related, relations, neighbors, explain_connection` ([TAP-1630](https://linear.app/tappscodingagents/issue/TAP-1630)).** `tapps_memory` exposes four new actions backed by the tapps-brain 3.17+ graph surface: `related` (→ `memory_find_related`), `relations` (→ `memory_relations` when called with `key`, otherwise `memory_query_relations` with subject/predicate/object_entity filters), `neighbors` (→ `brain_get_neighbors`, takes comma-separated `entry_ids`), and `explain_connection` (→ `brain_explain_connection` between `subject` and `object_entity`). The five corresponding tools join `_BRIDGE_USED_TOOLS` so profile mismatches surface through `tapps doctor` and `tapps_memory(action=health)`. In-process `BrainBridge` (legacy / no HTTP transport) returns a structured `knowledge_graph_requires_http_bridge` degraded payload instead of `AttributeError`. New params on `tapps_memory`: `subject`, `predicate`, `object_entity`, `max_hops`.
- **`feat(memory): batch ops route to memory_save_many family — save_bulk, recall_many, reinforce_many` ([TAP-1631](https://linear.app/tappscodingagents/issue/TAP-1631)).** `HttpBrainBridge` now overrides the inherited N-call `save_many` loop with the brain's single-round-trip `memory_save_many` endpoint. New `recall_many` and `reinforce_many` bridge methods + `tapps_memory` actions wrap `memory_recall_many` and `memory_reinforce_many`. The existing `save_bulk` MCP action, when running in HTTP-bridge mode, now batches all entries into one wire POST instead of N. The three new tool names (`memory_save_many`, `memory_recall_many`, `memory_reinforce_many`) join `_BRIDGE_USED_TOOLS`. Live brain contract test asserts the architectural win directly by counting POSTs (`50 entries -> 1 round trip`); the wall-clock 5x target from the original Linear acceptance is **not** met on tapps-brain 3.17.1 because the brain's batch handlers do not coalesce their underlying DB writes into a single transaction — that requires a follow-up brain-side change, which is out of scope per the epic's non-goals.
- **`feat(memory): feedback flywheel auto-emit + tapps-brain health diagnostics` ([TAP-1632](https://linear.app/tappscodingagents/issue/TAP-1632)).** `tapps_memory(action=search)` now auto-emits `feedback_gap` to tapps-brain when results are empty or — if `memory.feedback_min_similarity` is set — when the top hit's score falls below the threshold. The emit can be disabled with `memory.feedback_auto_emit: false`. New `tapps_memory(action=rate)` calls `feedback_rate` with `key`, `rating`, optional `session_id` / `details_json`. New bridge methods `feedback_rate`, `feedback_gap`, `flywheel_report`, `diagnostics_report`; all four tools join `_BRIDGE_USED_TOOLS`. `tapps doctor` adds a `tapps-brain health` row that pulls a compact summary from `flywheel_report` + `diagnostics_report` so operators see gap counts, rating counts, and brain-quality scores at a glance.
- **`feat(memory): native session memory replaces bespoke local index` ([TAP-1633](https://linear.app/tappscodingagents/issue/TAP-1633)).** `tapps_memory(action=index_session)` now routes through `bridge.index_session(...)` → the brain's `memory_index_session` tool instead of the legacy local `tapps_core.memory.session_index` path. New `tapps_memory(action=search_sessions)` maps to `memory_search_sessions`; new `tapps_memory(action=session_end)` maps to `tapps_brain_session_end` (summary via `value`, tags via the existing `tags` channel, daily-note flag via `dry_run`). The bespoke session-index merge in `_ranked_search` is removed — agents that previously passed `include_session_index=True` now call the explicit `search_sessions` action instead. Three new tool names join `_BRIDGE_USED_TOOLS`; `index_session` migrates from `_DISPATCH` (sync local-store) to `_ASYNC_DISPATCH` (bridge-routed). In-process bridges without the HTTP transport return a structured `native_sessions_require_http_bridge` degraded payload.
- **`feat(brain-bridge): structured McpError discriminator`.** [`brain_bridge.py`](packages/tapps-core/src/tapps_core/brain_bridge.py) — new `BrainMcpError(RuntimeError)` preserves the JSON-RPC `code`, `data`, and `tool_name` fields from the brain's error envelope so callers can discriminate between "tool gated by profile" (`data.error == "tool_not_in_profile"`, EPIC-073) and "tool removed from registry" without re-parsing the stringified message. Module-level `_classify_mcp_error(exc)` helper returns `"gated"` / `"removed"` / `"other"` and handles both direct exceptions and the `BrainBridgeUnavailable` retry-exhausted wrapper.
...
- **`feat(hooks): override autonomous-loop EXIT_SIGNAL on missing quality gate or checklist` ([TAP-1326](https://linear.app/tappscodingagents/issue/TAP-1326), [TAP-1327](https://linear.app/tappscodingagents/issue/TAP-1327)).** [`platform_hook_templates.py`](packages/tapps-mcp/src/tapps_mcp/pipeline/platform_hook_templates.py) — Stop hook now reads `loop status.json` and overrides `EXIT_SIGNAL=true` to `false` when modified `.py`/`.ts`/`.tsx`/`.go`/`.rs` files in the loop did not receive a `tapps_quick_check`/`tapps_validate_changed`/`tapps_quality_gate` call, or when `tapps_checklist` was not called. Logs `QUALITY_GATE_SKIP` / `CHECKLIST_MISSING` to `loop logs` and appends a TODO line to `loop PROMPT.md` for the next iteration. Removed in 3.12.28 — loop integration dropped from tapps-mcp.
- **`feat(hooks): record edits + flag new external imports in post-edit hook` ([TAP-1330](https://linear.app/tappscodingagents/issue/TAP-1330)).** PostToolUse hook on Edit/Write writes the modified path to `.tapps-mcp/.edits_this_loop` (consumed by the Stop hook above) and parses added Python/TS imports to surface a `tapps_lookup_docs` reminder for newly-introduced external libraries.
...
- **TAP-1329** (custom-agent.md surface brain_recall / security_scan / score_file) — deferred per maintainer direction; being moved to a different repo since `custom-agent.md` is owned by external agent projects rather than tapps-mcp templates.
...
- **`test: fix 16 pre-existing failures on master + add CI gate` ([TAP-1093](https://linear.app/tappscodingagents/issue/TAP-1093)).** Brought the tapps-mcp non-slow suite to **0 failures** (4246 passed, was 4227). All 16 fixes were stale assertions that pre-dated 3.5.0, surfaced by the autonomy-rule release-readiness pass. Clusters: hardcoded `== 26` tool count assertions vs reality 29 (7 tests in [`test_tool_annotations.py`](packages/tapps-mcp/tests/unit/test_tool_annotations.py) + [`test_enabled_tools_config.py`](packages/tapps-mcp/tests/unit/test_enabled_tools_config.py); refactored to derive from `len(ALL_TOOL_NAMES)` where idiomatic, and `test_no_tools_marked_destructive` swapped for a `_ALLOWED_DESTRUCTIVE` allowlist so cache eviction can legitimately mark `destructiveHint=True`); Claude hooks shape drift (5 tests in [`test_claude_hooks_generation.py`](packages/tapps-mcp/tests/unit/test_claude_hooks_generation.py); `UserPromptSubmit` event + `tapps-user-prompt-submit.sh` script were added in TAP-975 but invariants weren't refreshed); vulture command shape (2 tests in [`test_vulture.py`](packages/tapps-mcp/tests/unit/test_vulture.py); `--ignore-names=cls` flag added in TAP-616 / 3.4.0); brain http memory_status (mock `auth_probe` at the bridge boundary); PyInstaller spec (5 modules added: `tapps_mcp.server_linear_tools`, `tapps_mcp.pipeline.{migrate_memory,linear_sdlc,linear_sdlc.config,linear_sdlc.renderer}`).

### Internal

- **`ci: add tapps-mcp non-slow tests workflow`.** New [`.github/workflows/non-slow-tests.yml`](.github/workflows/non-slow-tests.yml) runs `uv run pytest packages/tapps-mcp/tests/ -m "not slow"` on every push/PR to `main`/`master` (30-min timeout, mirrors the [`agents-md-stamp.yml`](.github/workflows/agents-md-stamp.yml) shape). Prevents the kind of silent green-test drift that produced TAP-1093 — silently failing assertions on master are now visible at PR time rather than discovered during release prep.
...
- **`fix(loop hooks): allow git rm --cached and untrack runtime artifacts` (TAP-1020).** The PreToolUse `validate-command.sh` over-blocked any `git rm` invocation as destructive, even though `git rm --cached` only updates the index. Combined with the `protect-loop-files.sh` guard on the validate hook itself, agents could not clean up the four `loop runtime artifacts` runtime files that commit `90739d0` added to `.gitignore` -- every session opened with a dirty working tree. Carved out `git rm --cached` while keeping bare `git rm`, `git clean`, and `git reset --hard` blocked, then untracked the four index entries.
...
- **`fix(tests): correct generator fixture annotation to Iterator[None]` (TAP-697).** `_fresh_settings` in `test_upgrade_consumer_feedback.py` is a yield-based fixture but was annotated `-> None` -- a type lie that `mypy --strict` would flag if the `@pytest.fixture` decorator were not untyped. Fixed to `-> Iterator[None]`. Scope check across all package tests confirmed this was the only such occurrence.
...
- **Agent-facing clarity pass: every surface an agent reads now tells it *when* to call a tool and *what the benefit* is.** Fixes found during a sweep of CLI help output and `.mcp.json` `instructions` strings across 7 projects (tapps-mcp itself + 6 consumers).
  - **`.mcp.json` `instructions`** (this repo's tapps-mcp entry + the tapps-mcp / docs-mcp entries in AgentForge, Alpaca, tapps-brain, Workstation, everything-claude-code, external bash consumer): rewritten from feature lists to structured *trigger points* + *benefit* text so agents understand which user action each tool responds to. Previously read like a brochure; now reads like a decision table.
...
- **Documentation accuracy audit** — corrected tool counts across all user-facing docs: TappsMCP has **26 tools** (not 30), total platform is **58 tools** (not 62). Affected files: README.md, AGENTS.md, CLAUDE.md, CONTRIBUTING.md, ARCHITECTURE.md, ONBOARDING.md, TECH_STACK.md, docker readme, project AGENT.md, and all AGENTS.md templates (high/medium/low).
