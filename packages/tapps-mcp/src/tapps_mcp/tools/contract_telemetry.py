...
def record_tool_result_bytes(
    ...
    *,
    ...
    ``source`` distinguishes a direct MCP tool result (``"tool"``) from a
    delegated subagent result (``"subagent"``) -- both count toward the
    per-session byte/context-growth summary
    :func:`tapps_mcp.tools.usage.summarize_session_ledger` builds. ``success``
    records whether the recorded response was an error envelope, so an
    error-heavy session is visible in the ledger rather than silently absent.

    Unlike :func:`record_pipeline_mark`, an ``OSError`` here is **not**
    swallowed: the dispatch seam (``tapps_mcp.server._record_ledger_entry``)
    is the one place that catches and logs it, so a read-only ``.tapps-mcp/``
    directory produces a visible warning instead of a silently-empty ledger.
    """
    metrics_dir = project_root / ".tapps-mcp"
