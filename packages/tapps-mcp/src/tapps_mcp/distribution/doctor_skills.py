...
def check_orchestration_prompt_learnings_ceiling(project_root: Path) -> CheckResult:
    ...
    The emitted SKILL.md tells the agent "Past roughly 120 bullets or 40 KB,
    merge overlapping lines" — that was prose only, with nothing measuring it,
    until this check. ``learnings.md`` is create-only (never regenerated by
    upgrade), so it is the one file in the skill that can grow unbounded
    across sessions with no upgrade cycle ever touching it again.

    This is a minimal, standalone size/bullet-count check with no dependency
    on TAP-6861's fuller learnings.md audit (near-duplicate detection,
    contradiction detection, single-home verification), which is unmerged at
    the time this was written. Once that lands, this check should delegate to
    its ``size_finding()`` instead of carrying its own copy of the thresholds.
    """
    from tapps_mcp.distribution.doctor_pipeline import _tapps_skill_bases
    from tapps_mcp.distribution.doctor_result import CheckResult
    from tapps_mcp.pipeline.skill_managed_block import learnings_size_finding

    over_ceiling: list[str] = []
    checked = 0
    for host_label, base in _tapps_skill_bases(project_root):
        learnings_path = base / "orchestration-prompt" / "learnings.md"
    ...
    if over_ceiling:
        return CheckResult(
            ...
            False,
            f"{len(over_ceiling)} learnings.md past ceiling: {', '.join(over_ceiling)}",
            "Merge overlapping lines and delete ones overtaken by a fixed tool "
            "or a changed codebase (see the skill's own 'Keep it readable' rule).",
            severity="warn",
        )
    if not checked:
        return CheckResult(
            "orchestration-prompt learnings ceiling", True, "no learnings.md installed"
        )
    return CheckResult(
        "orchestration-prompt learnings ceiling",
        True,
        f"{checked} learnings.md file(s) under ceiling",
    )
