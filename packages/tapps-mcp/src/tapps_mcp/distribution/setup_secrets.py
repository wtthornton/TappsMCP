...
def ensure_tapps_runtime_gitignore(project_root: Path) -> list[str]:
    ...
    entries under that tree. Creates ``.gitignore`` with the runtime entries
    when the file is absent — otherwise upgrade backups (hundreds of files
    under ``.tapps-mcp/backups/``) become committable in consumer repos
    scaffolded without a ``.gitignore``.
    ...
    if not gitignore.exists():
        body = (
            "# TappsMCP runtime/upgrade artifacts (generated by tapps-mcp)\n"
            + "\n".join(_TAPPS_RUNTIME_GITIGNORE_ENTRIES)
            + "\n"
        )
        try:
            ...
            return []
        return list(_TAPPS_RUNTIME_GITIGNORE_ENTRIES)
    try:
        text = gitignore.read_text(encoding="utf-8")
    except OSError:
        return []
