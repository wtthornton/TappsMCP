...
1. `linear_list_issues` with **explicit narrowing filters** — always pass `team`, `project`, `state` (`"backlog"` or `"unstarted"`), and `includeArchived=false`. Add `label` when the user asks about a specific label. Never call without filters: broad fetches waste Linear quota and cache poorly.
2. If the user names a specific issue (e.g. "triage TAP-686"), use `linear_get_issue(id="TAP-686")` instead — skip `linear_list_issues` entirely.
3. Pass the list to `docs_linear_triage`.
4. Present label_proposals, parent_groupings, and metadata_gaps. Confirm with user before applying any changes.
