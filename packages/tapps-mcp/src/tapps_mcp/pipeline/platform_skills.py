...
1. **Prefer** `tapps_linear_snapshot(team=<team>, project=<project>, state="backlog" | "unstarted", label?)` — read-through cached; 5min TTL for open issues, 1h for closed. Cache hits return in <50ms and do not count against Linear API quota. Required args: `team`, `project`.
2. Fall back to `linear_list_issues` only when `tapps_linear_snapshot` returns `degraded: true` (e.g. `TAPPS_MCP_LINEAR_API_KEY` unset). In that case, pass the same narrowing filters — `team`, `project`, `state`, `includeArchived=false` — never call without filters.
3. If the user names a specific issue (e.g. "triage TAP-686"), use `linear_get_issue(id="TAP-686")` instead — skip list/snapshot entirely.
