...
# TappsMCP PostToolUse hook — session-start sentinel writer.
# Writes the sentinel ONLY when tapps_session_start actually returned a
# success envelope (TAP-7018: a tool_relocated pointer error used to
# satisfy this gate on tool name alone).
$stdin = [Console]::In.ReadToEnd()
$tool = ''; $sid = ''; $ok = $false
try {
    $d = $stdin | ConvertFrom-Json
    ...
    $resp = $d.tool_response
    if (-not $resp) { $resp = $d.toolResponse }
    if ($resp -is [string]) {
        try { $resp = $resp | ConvertFrom-Json } catch { $resp = $null }
    }
    if ($resp -and ($resp.PSObject.Properties.Name -contains 'success') `
        -and $resp.success -eq $true `
        -and -not ($resp.PSObject.Properties.Name -contains 'error')) {
        $ok = $true
    }
} catch { exit 0 }
