...
def generate_claude_agent_to_agent_rule(
    project_root: Path,
) -> dict[str, Any]:
    """Generate ``.claude/rules/agent-to-agent.md``.

    On-demand rule (``alwaysApply: false``) documenting the agent-to-agent
    transport, identity/authority, acceptance, coordination, epistemic, and
    message-discipline protocol for Claude Code sessions on one machine.
    Mirrors the deployment shape of :func:`generate_claude_agent_scope_rule`.
    Idempotent — re-running overwrites with the same content. TAP-6886.
    ...
    rules_dir = project_root / ".claude" / "rules"
    rules_dir.mkdir(parents=True, exist_ok=True)
    target = rules_dir / "agent-to-agent.md"
    return _write_claude_rule_file(target, _CLAUDE_AGENT_TO_AGENT_RULE)
