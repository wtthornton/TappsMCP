...
def generate_claude_hooks(
    ...
    *,
    ...
    if engagement_level == "high":
        blocking = (
            _CLAUDE_HOOK_SCRIPTS_BLOCKING_PS if win
            else _CLAUDE_HOOK_SCRIPTS_BLOCKING
        )
    ...
    for event, entries in hooks_config.items():
        if event not in existing_hooks:
            ...
            existing_matchers = {
                e.get("matcher") for e in existing_hooks[event]
    ...
    action = (
        "migrated" if hooks_migrated > 0
        else ("created" if hooks_added > 0 else "skipped")
    )
    return {
    ...
    *,
    ...
    for event, entries in existing_hooks.items():
        for i, entry in enumerate(entries):
            ...
            if (
                _is_wrong_platform_command(cmd, win=win)
                and event in hooks_config
            ):
