...
<!-- tapps-skill-asset-project-customizations: nlt-orchestrator additions. Everything below this line
survives tapps_upgrade; everything above it is regenerated. 2026-09-04: the previous content here was a
byte-for-byte duplicate of the managed block above (18 headings, 0 unique — verified with comm before
deletion); it was preserved wholesale by the marker migration and then frozen while the upper copy
advanced. Do not paste the managed block here again — add only sections the upstream template lacks. -->

## Lane briefs  (nlt-orchestrator — the execution unit is a dispatched lane, so name them)

<Every code change in a fleet repo runs as `LANE_ISSUE=<TAP> scripts/dispatch-lane.sh <repo> <brief>
<model> <effort> [budget]` with the repo as cwd in its own worktree. The driver prompt never restates a
brief; it points at the file. A row with no brief file is a lane nobody can dispatch.>

| Lane | Repo · worktree | Issue | Brief | Wave | model · effort (lane CLI) |
|------|-----------------|-------|-------|------|---------------------------|
| L1 <name> | <repo> · `/tmp/<slug>-l1` | TAP-#### | `prompts/<slug>-lane-l1-*.md` | 1 | `sonnet` · `medium` |

- **Two `effort` knobs, not one.** The Plane map's `effort` column is Workflow `opts.effort`; a lane's
  effort is `dispatch-lane.sh`'s fourth argument and reaches `claude -p --effort`. An Agent-tool
  subagent has neither. Say which one a cell means — an all-`—` effort column with lanes present is
  not "surrendered control", it is the wrong column.
- **Each brief carries, verbatim:** the tracking issue, the goal, *the one thing that will silently give
  you a wrong result*, the measured starting state to re-paste, the exact file partition, the forbidden
  solutions, the acceptance boxes with their negative controls, and the `--- LINEAR EVIDENCE ---`
  `linear_write=not-attempted` boundary (the plugin is unreachable inside `claude -p`).
- **Standing constraints are injected by `dispatch-lane.sh`**, not hand-copied into briefs
  (`lane-constraints-live-in-dispatch`); a brief restates only what is specific to its lane.

## Validation contract — the six-column form (nlt-orchestrator; replaces the managed four-column table)

<The managed table above has `ID | assertion | sub-goal | tool`. Every verify Workflow in this repo
(`val-verify.js`, `cce-leverage-verify.js`) REFUSES a VAL without a `kind`, a proof command, a
negative control, and a positive control — so a four-column contract forces the driver to invent
the controls at verify time, in its own context, at frontier tier. Write the contract in the shape
the Workflow reads. `kind` picks the verifier tier: deterministic → haiku/low · comparative →
sonnet/medium · semantic → opus/high. `check-prompt-shape.js` fails a prompt whose contract lacks
the `kind` column or never names both controls.>

| ID | Behavioral assertion | Fulfilled by | kind | Proof command | Negative control | Positive control |
|----|----------------------|--------------|------|---------------|------------------|------------------|
| VAL-01 | <outcome> | <lane/sub-goal> | deterministic | `<cmd>` → exit 0 | `<cmd on a broken input>` → must FAIL | `<cmd on a known-good input>` → must PASS |

- Coverage rule: every id claimed exactly once; Done-when requires all ids green; a control that
  does not discriminate is `blocked`, not green.
- Hand this table to the Workflow as real JSON `args.vals` — never as prose in the prompt string.
