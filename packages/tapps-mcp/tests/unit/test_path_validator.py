...
from tapps_mcp.security.path_validator import PathValidator, assert_write_allowed
...
class TestPathValidator:
    def test_validate_path_within_root(self, validator, tmp_project):
        f = tmp_project / "src" / "main.py"
        result = validator.validate_path(f, must_exist=True)
        assert result == f.resolve()

    def test_validate_path_outside_root_raises(self, validator, tmp_path):
    ...
    def test_validate_path_not_found_raises(self, validator, tmp_project):
        with pytest.raises(FileNotFoundError):
            validator.validate_path(tmp_project / "missing.py", must_exist=True)

    def test_validate_path_not_found_ok_when_not_required(self, validator, tmp_project):
        result = validator.validate_path(tmp_project / "new.py", must_exist=False)
        assert result == (tmp_project / "new.py").resolve()

    def test_validate_path_too_large(self, validator, tmp_project):
    ...
    def test_validate_path_size_ok_when_disabled(self, validator, tmp_project):
        big = tmp_project / "big.bin"
        big.write_bytes(b"x" * 100)
        result = validator.validate_path(big, must_exist=True, max_file_size=None)
        assert result == big.resolve()

    def test_validate_read_path(self, validator, tmp_project):
        f = tmp_project / "src" / "main.py"
        assert validator.validate_read_path(f) == f.resolve()

    def test_validate_write_path(self, validator, tmp_project):
...
class TestAssertWriteAllowed:
    def test_write_within_root(self, tmp_project):
        assert_write_allowed(tmp_project / "src" / "new.py", tmp_project)

    def test_write_outside_root_raises(self, tmp_project):
        with pytest.raises(PathValidationError):
            assert_write_allowed(tmp_project.parent / "hack.py", tmp_project)

    def test_write_with_allowed_prefix(self, tmp_project):
        assert_write_allowed(
            tmp_project / "src" / "new.py", tmp_project, allowed_prefixes=["src", "tests"]
        )

    def test_write_with_disallowed_prefix(self, tmp_project):
