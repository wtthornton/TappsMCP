...
from tapps_mcp.adaptive.models import ExpertPerformance, ExpertWeightMatrix
from tapps_mcp.adaptive.persistence import FilePerformanceTracker
from tapps_mcp.adaptive.voting_engine import (
...
class TestAdjustWeights:
    ...
    async def test_none_matrix_builds_default(self, engine: AdaptiveVotingEngine) -> None:
        with patch(
            "tapps_mcp.adaptive.voting_engine._build_default_matrix"
        ) as mock_build:
...
class TestBuildDefaultMatrix:
    @patch("tapps_mcp.adaptive.voting_engine.WeightDistributor")
    def test_build_from_registry(self, mock_wd: MagicMock) -> None:
        ...
        with patch(
            "tapps_mcp.experts.registry.ExpertRegistry.get_all_experts",
