...
class TestReportCtxInfo:
    ...
    async def test_ctx_info_called_per_file(self) -> None:
        ...
        with patch("tapps_mcp.server_analysis_tools._record_call"), \
            ...
            with patch("tapps_mcp.gates.evaluator.evaluate_gate", return_value=mock_gate), \
                 ...
                 patch("pathlib.Path.rglob", return_value=[Path("/fake/project/a.py"), Path("/fake/project/b.py")]), \
    ...
    async def test_ctx_noop_when_none(self) -> None:
        ...
        with patch("tapps_mcp.server_analysis_tools._record_call"), \
             ...
             patch("tapps_mcp.server_analysis_tools.ensure_session_initialized", new_callable=AsyncMock):
            from pathlib import Path
            mock_settings.return_value = MagicMock(project_root=Path("/fake"), quality_preset="standard")
            mock_scorer = MagicMock()
