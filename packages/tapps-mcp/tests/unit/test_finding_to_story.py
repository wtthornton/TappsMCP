...
# ---------------------------------------------------------------------------
# TAP-2721: Duplicate detection against Linear snapshots
# ---------------------------------------------------------------------------


class TestFindDuplicateInSnapshot:
    """Unit tests for the pure find_duplicate_in_snapshot() helper."""

    def _make_issues(self, *titles: str) -> list[dict[str, str]]:
        return [{"id": f"TAP-{i}", "title": t} for i, t in enumerate(titles, start=100)]

    def test_exact_match_returns_issue(self) -> None:
        from tapps_mcp.tools.finding_to_story import find_duplicate_in_snapshot

        issues = self._make_issues("security: use parameterised queries")
        result = find_duplicate_in_snapshot("security: use parameterised queries", issues)
        assert result is not None
        assert result["id"] == "TAP-100"

    def test_no_match_returns_none(self) -> None:
        from tapps_mcp.tools.finding_to_story import find_duplicate_in_snapshot

        issues = self._make_issues("security: something else entirely")
        result = find_duplicate_in_snapshot("style: remove unused import", issues)
    ...
    def test_punctuation_normalised(self) -> None:
        ...
        # Ellipsis in generated title (from truncation) vs clean existing title
        issues = self._make_issues("correctness: fix the null dereference at line")
        result = find_duplicate_in_snapshot(
            "correctness: fix the null dereference at line\u2026", issues
        )
        assert result is not None
    ...
    def test_empty_issues_returns_none(self) -> None:
        ...
        assert find_duplicate_in_snapshot("security: any title", []) is None

    def test_empty_title_returns_none(self) -> None:
        from tapps_mcp.tools.finding_to_story import find_duplicate_in_snapshot

        issues = self._make_issues("security: any title")
        assert find_duplicate_in_snapshot("", issues) is None

    def test_first_match_returned(self) -> None:
        from tapps_mcp.tools.finding_to_story import find_duplicate_in_snapshot

        issues = self._make_issues("style: remove unused import", "style: remove unused import")
        result = find_duplicate_in_snapshot("style: remove unused import", issues)
        assert result is not None
        assert result["id"] == "TAP-100"  # first one

    def test_missing_title_field_skipped(self) -> None:
        from tapps_mcp.tools.finding_to_story import find_duplicate_in_snapshot

        issues: list[dict[str, str]] = [{"id": "TAP-100"}]  # no title key
        assert find_duplicate_in_snapshot("security: fix", issues) is None

    def test_generated_title_deduped_via_round_trip(self) -> None:
        """A title generated by finding_to_story should dedup against itself."""
        from tapps_mcp.tools.finding_to_story import find_duplicate_in_snapshot, finding_to_story
        ...
        issues = [{"id": "TAP-999", "title": story.title}]
        result = find_duplicate_in_snapshot(story.title, issues)
        assert result is not None
        assert result["id"] == "TAP-999"


class TestTappsFindingToStoryHandlerDedup:
    """TAP-2721: handler dedup via snapshot_issues_json param."""

    @pytest.mark.asyncio()
    async def test_no_snapshot_always_files(self) -> None:
        from tapps_mcp.server_analysis_tools import tapps_finding_to_story
        ...
        assert result["data"]["should_file"] is True
        assert result["data"]["duplicate_found"] is False
        assert result["data"]["duplicate_of"] is None

    @pytest.mark.asyncio()
    async def test_snapshot_no_match_files(self) -> None:
        import json

        from tapps_mcp.server_analysis_tools import tapps_finding_to_story

        snapshot = json.dumps([{"id": "TAP-500", "title": "correctness: unrelated issue"}])
        result = await tapps_finding_to_story(
            severity="P2",
            category="style",
            files=["src/x.py:1"],
            evidence="unused import",
            recommendation="remove unused import",
            snapshot_issues_json=snapshot,
        )
        assert result["data"]["should_file"] is True
        assert result["data"]["duplicate_found"] is False

    @pytest.mark.asyncio()
    async def test_snapshot_match_skips_filing(self) -> None:
        import json

        from tapps_mcp.server_analysis_tools import tapps_finding_to_story

        # First generate the title, then put it in a snapshot
        result_no_snap = await tapps_finding_to_story(
            severity="P1",
            category="security",
            files=["src/auth.py:10-50"],
            evidence="missing auth check at line 20",
        ...
        generated_title = result_no_snap["data"]["title"]
        snapshot = json.dumps([{"id": "TAP-555", "title": generated_title}])
        ...
        assert result["data"]["should_file"] is False
        assert result["data"]["duplicate_found"] is True
        assert result["data"]["duplicate_of"] == "TAP-555"

    @pytest.mark.asyncio()
    async def test_malformed_snapshot_json_proceeds_safely(self) -> None:
        from tapps_mcp.server_analysis_tools import tapps_finding_to_story
        ...
        result = await tapps_finding_to_story(
            ...
            evidence="unused import",
            recommendation="remove unused import",
            snapshot_issues_json="not valid json {{{",
        )
        # Malformed JSON → dedup skipped → proceed to file
        assert result["success"] is True
        assert result["data"]["should_file"] is True

    @pytest.mark.asyncio()
    async def test_next_steps_mention_duplicate_id_on_match(self) -> None:
        import json
        ...
        generated_title = result_no_snap["data"]["title"]
        snapshot = json.dumps([{"id": "TAP-777", "title": generated_title}])
        ...
        next_steps = result["data"].get("next_steps", [])
        assert any("TAP-777" in s for s in next_steps)
