...
A fleet audit found: (1) tapps-mcp-workflow-fix is 3 majors stale at 2.10.1; (2) the source repo itself has a self-stamp drift (AGENTS.md says 3.2.4 vs shipped 3.3.0); (3) the only blocking hook (PreToolUse) is missing from 3/5 consumers; (4) a PostToolUseFailure matcher is wired with no handler script; (5) there is no composite /tapps-finish-task skill so agents routinely drop the closing sequence; (6) git commits bypass all validation. Without closing these gaps, "projects are commonized" is an aspiration, not a fact.
