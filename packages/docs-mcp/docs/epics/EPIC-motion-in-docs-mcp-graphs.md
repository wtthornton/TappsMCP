...
Convert hand-built `<line>` edges to `<path id=...>` and append `<circle><animateMotion mpath/></circle>` on the flow-meaningful panels in architecture.py and pattern_poster.py.
