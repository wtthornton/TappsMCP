...
# ===================================================================
# Story 62.6 — Settings tests
# ===================================================================


class TestDocValidationSettings:
    """Tests for MemoryDocValidationSettings."""

    def test_default_settings(self) -> None:
        from tapps_core.config.settings import MemoryDocValidationSettings

        s = MemoryDocValidationSettings()
        assert s.enabled is False
        assert s.validate_on_session_start is True
        assert s.max_entries_per_session == 5
        assert s.confidence_threshold == 0.5
        assert s.dry_run is False

    def test_memory_settings_includes_doc_validation(self) -> None:
        from tapps_core.config.settings import MemorySettings

        ms = MemorySettings()
        assert hasattr(ms, "doc_validation")
        assert ms.doc_validation.enabled is False
    ...
    def test_custom_settings(self) -> None:
        ...
        assert s.max_entries_per_session == 10
        assert s.confidence_threshold == 0.3
        assert s.dry_run is True


# ===================================================================
# Story 62.6 — Session-start validation tests
# ===================================================================


class TestSessionStartValidation:
    """Tests for _maybe_validate_memories in server_pipeline_tools."""
    ...
    def _make_settings(
        ...
        *,
        ...
        settings = MagicMock()
        settings.memory.doc_validation = MemoryDocValidationSettings(
            enabled=enabled,
            validate_on_session_start=validate_on_start,
    ...
    async def test_skipped_when_disabled(self) -> None:
        from tapps_mcp.server_pipeline_tools import _maybe_validate_memories
    ...
    @pytest.mark.asyncio()
    async def test_skipped_when_already_done(self) -> None:
    ...
    @pytest.mark.asyncio()
    async def test_skipped_when_no_memory_settings(self) -> None:
        from tapps_mcp.server_pipeline_tools import _maybe_validate_memories

        settings = MagicMock(spec=[])
        result = await _maybe_validate_memories(store=MagicMock(), settings=settings)
        assert result is None

    @pytest.mark.asyncio()
    async def test_runs_once_per_session(self) -> None:
        from tapps_mcp.server_pipeline_tools import (
            _maybe_validate_memories,
            _session_state,
        )
        ...
        result2 = await _maybe_validate_memories(store, settings)
