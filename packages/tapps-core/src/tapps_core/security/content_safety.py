"""Content safety - prompt injection detection for retrieved documentation.

All documentation retrieved from external sources (Context7 API, cached
files, bundled docs) passes through this filter before being returned to
the LLM.  Retrieved content is *untrusted* - it may contain prompt
injection attempts embedded in documentation.

Also used by the memory subsystem to sanitise stored content.

Previously located in ``knowledge.rag_safety``; moved here to break the
circular dependency between ``memory`` and ``knowledge`` packages.
"""
...
def check_content_safety(content: str) -> SafetyCheckResult:
    """Check retrieved documentation content for prompt injection.
