...
class TappsMCPSettings(BaseSettings):
    ...
    linear_cache_ttl_closed_seconds: int = Field(
        ...
        ge=0,
        description=(
            "TTL for cached closed/archived Linear issues stored via "
            "tapps_linear_snapshot_put (seconds). 0 disables caching."
        ),
    )
    ...
    linear_enforce_cache_gate: Literal["off", "warn", "block"] = Field(
        ...
        description=(
            ...
            "before tightening. 'block' rejects raw Linear MCP `list_issues` "
            "calls (server id varies by host) that lack a recent "
    ...
    linear_team: str = Field(
        default="",
        description=(
            "Linear team name or ID used by tapps_release_update to populate "
            "the team field in the response. When empty, the agent must supply "
            "the team parameter directly."
        ),
    )
    linear_project: str = Field(
        default="",
        description=(
            "Linear project name or slug used by tapps_release_update to "
            "populate the project field in the response (passed as the "
            "'project' arg to save_document). When empty, the agent must "
            "supply the project parameter directly."
        ),
    )
    release_update_extra_sections: list[str] = Field(
        default_factory=list,
        description=(
            "Additional markdown sections to append to every release update "
            "body generated by tapps_release_update. Each string is a "
            "raw markdown block (e.g. '### Testing Notes\\n- Ran regression suite')."
        ),
    )
    release_update_footer: str = Field(
        default="",
        description=(
            "Footer line appended to every release update body. "
            "Use for project-specific links or standard sign-offs."
        ),
    )
    release_update_custom_links: dict[str, str] = Field(
        default_factory=dict,
        description=(
            "Extra link entries added to the '### Links' section of every "
            "release update body. Merged after auto-detected links. "
            "Keys are labels, values are URLs."
        ),
    )
