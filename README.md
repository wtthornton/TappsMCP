...
**Tapps Platform** ships two MCP servers for AI-assisted development: **TappsMCP** (code quality, security, shared memory) and **DocsMCP** (documentation generation and maintenance). Together they expose **89 tools** (47 TappsMCP + 42 DocsMCP) with structured, deterministic outputs suitable for Claude Code, Cursor, VS Code, and any MCP host.
...
```
tapps-brain (standalone)  <──  tapps-core (shared infra)  <──  tapps-mcp (47 tools)
                                                          <──  docs-mcp  (42 tools)
                                                                      = 85 MCP tools
```
...
- **89 deterministic MCP tools** (47 TappsMCP + 42 DocsMCP) — no LLM calls in the tool chain; same input always produces same output
...
The platform exposes **89 MCP tools** (47 TappsMCP + 42 DocsMCP) plus workflow prompts. All tools are **deterministic** (no LLM calls in the tool chain).
...
│
├── tapps-mcp/                         # Code quality MCP server (47 tools)
│   └── src/tapps_mcp/
