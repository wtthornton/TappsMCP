...
| `session_start_gate` | string | `"warn"` (eng. default) | Enforce that `tapps_session_start` actually runs before other TappsMCP quality tools. The SessionStart hook can only *prompt* the agent (a hook can't execute an MCP tool); this gate makes it enforceable. `off` = no hooks installed; `warn` = log to `.tapps-mcp/.session-start-gate-violations.jsonl` and allow; `block` = reject (exit 2) any TappsMCP quality tool called before a tool-written `.session-start-done-<session_id>` sentinel exists (written by a PostToolUse hook *after* `tapps_session_start` returns — proving the tool ran, not merely that the hook fired). `tapps_session_start` / `server_info` / `doctor` / `usage` / `stats` are always allowed so the gate can't deadlock a fresh or broken session; unidentifiable sessions (no `session_id`) fail open. Also re-prompts for `tapps_session_start` after context compaction. Default `warn` at `high`/`medium` engagement, `off` at `low`. Bypass with `TAPPS_SKIP_SESSION_START_GATE=1` (logged). `tapps doctor` reports current mode + 24-h violation count. |
