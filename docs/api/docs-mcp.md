...
**Returns:** `str` - str - The rendered changelog markdown.
...
| Name | Type | Default |
|------|------|---------|
| `VALID_TYPES` | `ClassVar[frozenset[str]]` | `frozenset({'dependency', 'class_hierarchy', 'module_map', 'er_diagram', 'c4_context', 'c4_container', 'c4_component', 'sequence', 'pattern_card', 'pattern_comparison'})` |
...
```python
def generate(self, project_root: Path, *, diagram_type: str = 'dependency', output_format: str = 'mermaid', scope: str = 'project', depth: int = 2, direction: str = 'TD', show_external: bool = False, flow_spec: str = '', theme: str = 'default') -> DiagramResult:
```
...
|------|------|-------------|---------|
| `project_root` | `Path` | Root directory of the project. | *required* |
| `diagram_type` | `str` | One of ``dependency``, ``class_hierarchy``, ``module_map``, ``er_diagram``, ``c4_context``, ``c4_container``, ``c4_component``, or ``sequence``. | 'dependency' |
| `output_format` | `str` | Output format -- ``mermaid``, ``plantuml``, or ``d2``. | 'mermaid' |
...
| `flow_spec` | `str` | JSON string defining a manual sequence flow. Expected format: ``{"participants": ["A", "B"], "messages": [{"from": "A", "to": "B", "label": "call"}]}``. When empty, auto-detects from import graph entry points. | '' |
| `theme` | `str` | D2 theme -- ``default``, ``sketch``, or ``terminal``. Ignored for mermaid and plantuml formats. | 'default' |

**Returns:** `DiagramResult` - A :class:`DiagramResult` with the rendered content.
...
# `packages.docs-mcp.src.docs_mcp.generators.doc_index`
...
```python
def generate(self, project_root: Path, *, doc_dirs: list[str] | None = None, output_path: str | None = None) -> DocIndexResult:
```
...
|------|------|-------------|---------|
| `project_root` | `Path` | Root directory of the project. | *required* |
| `doc_dirs` | `list[str] \| None` | Optional list of directories to scan. When empty, scans the entire project. | None |
| `output_path` | `str \| None` | Project-root-relative path where the generated index will be written. When set, link targets in the rendered markdown are made relative to the index's parent directory so markdown viewers resolve them correctly. When None, targets remain project-root-relative (legacy behavior). | None |

**Returns:** `DocIndexResult` - DocIndexResult with the markdown index.
...
| Name | Type | Default |
|------|------|---------|
| `VALID_STYLES` | `ClassVar[frozenset[str]]` | `frozenset({'minimal', 'standard', 'comprehensive', 'auto'})` |
| `VALID_STATUSES` | `ClassVar[frozenset[str]]` | `frozenset({'Proposed', 'In Progress', 'Complete', 'Blocked', 'Cancelled'})` |
...
```python
def generate(self, config: EpicConfig, *, project_root: Path | None = None, auto_populate: bool = False, quick_start: bool = False) -> str:
```
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `config` | `EpicConfig` | Epic configuration with title, stories, etc. | *required* |
...
| `quick_start` | `bool` | When True, infer defaults from the title for empty fields. Explicit parameters are never overwritten. | False |

**Returns:** `str` - Rendered markdown content with docsmcp markers.

#### `generate_with_timing`

```python
def generate_with_timing(self, config: EpicConfig, *, project_root: Path | None = None, auto_populate: bool = False, quick_start: bool = False) -> tuple[str, dict[str, int]]:
```

Like :meth:[`generate`](#generate) but returns per-phase timings (milliseconds).
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `config` | `EpicConfig` |  | *required* |
| `project_root` | `Path \| None` |  | None |
| `auto_populate` | `bool` |  | False |
| `quick_start` | `bool` |  | False |

**Returns:** `tuple[str, dict[str, int]]`

#### staticmethod `parse_stories_json`
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `stories_json` | `str \| list[Any]` | JSON string or pre-parsed list of story dicts. | *required* |

**Returns:** `list[EpicStoryStub]` - List of EpicStoryStub objects.
...
# `packages.docs-mcp.src.docs_mcp.generators.expert_utils`
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `answer` | `str` | Full consultation answer text (may start with ## header and boilerplate). | *required* |
| `max_length` | `int` | Maximum length of the returned advice string (default 300). | 300 |

**Returns:** `str` - First non-header, non-boilerplate paragraph, trimmed to max_length; empty if none.
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `guidance` | `list[dict[str, Any]]` |  | *required* |
...
# `packages.docs-mcp.src.docs_mcp.generators.frontmatter`
...
| Name | Type | Default |
|------|------|---------|
| `MANAGED_FIELDS` | `ClassVar[frozenset[str]]` | `frozenset({'title', 'description', 'category', 'diataxis_type', 'last_modified', 'tags'})` |
...
```python
def generate(self, content: str, *, file_path: Path | None = None, extra_fields: dict[str, Any] | None = None) -> FrontmatterResult:
```
...
|------|------|-------------|---------|
| `content` | `str` | The markdown file content. | *required* |
| `file_path` | `Path \| None` | Optional path for tag/category inference. | None |
| `extra_fields` | `dict[str, Any] \| None` | Additional fields to include in frontmatter. | None |

**Returns:** `FrontmatterResult` - FrontmatterResult with updated content and field tracking.
