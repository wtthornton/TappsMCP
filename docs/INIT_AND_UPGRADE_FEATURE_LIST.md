...
| **Skill tier** | `skill_tier="core" \| "full"` (`.tapps-mcp.yaml`) | Which skills init/upgrade deploy. `core` ≈ finish-task, handoff/continue, memory, tool-reference, research, security, init/upgrade/engagement, apply-files, linear-issue/read. `full` = entire registry (default). Wizard persists the choice; upgrade prunes managed non-core skills only when `skill_tier: core`. See [ADR-0031](adr/0031-always-on-context-budget.md). |
| **Context budget (doctor)** | `doctor_context_budget.*` in `.tapps-mcp.yaml` | Advisory WARN ceilings for CLAUDE.md / AGENTS.md bytes, alwaysApply rule weight, skill count, and SKILL.md line length. Mirrors `doctor_tool_budget_limit` for MCP tools. |
...
| **Hooks generation** | `platform="claude"` or `"cursor"` | **Claude:** Creates hook scripts in `.claude/hooks/` (including `tapps-session-start.sh` which kills stale MCP processes) and merges hook config into `.claude/settings.json`. Optional PreToolUse gates (`destructive_guard`, `linear_enforce_gate`, `linear_enforce_cache_gate`, `install_git_hooks`) are wired when the corresponding `.tapps-mcp.yaml` flags are set at init. **Cursor:** Creates 3 observability/reminder hook scripts in `.cursor/hooks/` and merges into `.cursor/hooks.json`. Cursor does **not** implement Claude-style PreToolUse block gates — Linear routing and destructive-command enforcement rely on skills + AGENTS.md obligations instead. |
| **Permission safety** | (auto, no flag) | `.claude/settings.json` denyList includes `Read(**/__pycache__/**)`, `Read(.venv/**)`, `Read(**/*.egg-info/**)` to prevent reads of cache/virtual-env artifacts. `BASH_MAX_OUTPUT_LENGTH=150000` caps verbose Bash output. |
...
| **Skills generation** | `platform="claude"` or `"cursor"` | Creates tapps-* skill directories (including `tapps-finish-task`, `tapps-handoff-session`, `tapps-continue-session`, linear-*, etc.) in `.claude/skills/` or `.cursor/skills/`. Skips existing files unless `tapps_upgrade --force`. After generation, `tapps_init` records `session_handoff_skills` ok/message in the result. |
