...
| **Hooks generation** | `platform="claude"` or `"cursor"` | **Claude:** Creates 7 hook scripts in `.claude/hooks/` (including `tapps-session-start.sh` which kills stale MCP processes) and merges hook config into `.claude/settings.json`. **Cursor:** Creates 3 hook scripts in `.cursor/hooks/` and merges into `.cursor/hooks.json`. Existing entries are preserved. |
