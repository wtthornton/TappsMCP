...
Cursor has a **40-tool limit** per MCP server. The combined server has **62** tools (30 TappsMCP + 32 DocsMCP), which exceeds this limit. Use separate servers:
...
Cursor enforces a 40-tool limit per MCP server. The combined TappsPlatform server has **62** tools and will be silently truncated. Use separate servers in Cursor (see configuration above).
