...
`tapps_session_start` returns a `data.brain_bridge_health` block on every call so agents can verify the brain is reachable before issuing memory writes. The same probe powers `tapps doctor`'s brain-health row.

| Field | Meaning |
|-------|---------|
| `enabled` | True when the memory pipeline is configured for this project. |
| `ok` | Roll-up — True only when bridge can reach the brain *and* pass its native `health` self-check. |
| `dsn_reachable` | HTTP mode: brain endpoint responded to a probe. In-process mode: pool was constructible. |
| `pool_config_valid` | Connection-pool sizing / DSN parsed cleanly. |
| `native_health_ok` | Result of the brain's own `health` tool — covers schema, embeddings, indexes. |
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `errors: ["brain_auth_failed"]`, HTTP 401 / 403 | Brain server requires a bearer token, none supplied. | Set `TAPPS_BRAIN_AUTH_TOKEN` (expanded into `TAPPS_MCP_MEMORY_BRAIN_AUTH_TOKEN` via `.mcp.json`) or export `TAPPS_MCP_MEMORY_BRAIN_AUTH_TOKEN` for CLI doctor. See [CONSUMER-REPO-BRAIN-WIRING.md](operations/CONSUMER-REPO-BRAIN-WIRING.md). For offline workflows, set `memory.tolerate_brain_auth_failure: true`. |
...
| `details.brain_version` below the floor (e.g. `3.23.x`) | Installed brain is older than the version pinned by tapps-mcp. | Upgrade tapps-brain — current floor is **3.24.0** per [ADR-0013](adr/0013-pin-tapps-brain-version-floor-at-3240.md). |
...
| **Handoff recall** | `memory get --key session-handoff` | Returns `handoff_sections` (Done/Open/P0) without embedding vector. Same enrichment applies to MCP `tapps_memory(action="get")`. |
| **Memory groups** | `memory save --memory-group insights` | Optional brain `memory_group` scope; CLI/MCP save omit `memory_group_note` when set. |
