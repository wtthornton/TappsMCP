...
**Handoff slots and the ownership guard (TAP-6871/6872).** A shared default file lets concurrent programs in the same repo clobber one another's cold-start channel with no trace. `tapps_handoff_save` (and CLI `handoff write`) accept an optional `slot` that namespaces both halves of the write — file `.tapps-mcp/handoffs/<slot>.md` and brain key `session-handoff.<slot>` — through the single naming site `handoff_schema.handoff_path()` / `handoff_memory_key()`; omitting `slot` reproduces the pre-slot path and key exactly. Every write, slotted or not, goes through `handoff_guard.guarded_write()`, which runs in this order:

1. Read the incumbent file at the target path (if any) and fingerprint it as `{program, updated, linear_p0, title}` from its `**Program:**` / `**Updated:**` / `**Linear P0:**` header lines and `# ` title.
2. Fingerprint the incoming write the same way (an `owner=` parameter can override the incoming `program` without touching the markdown body).
3. Classify the pair: `False` (same program, or the incumbent is older than `handoff_conflict_window_hours`), `True` (different stated programs, incumbent recent — the only value `block` mode refuses on), or `"unknown"` (either side is missing the header, or the incumbent has no parseable `Updated`). A handoff written before the `**Program:**` header existed always classifies `"unknown"` — reported, never blocked.
4. Under `handoff_conflict_mode: block`, a `True` classification raises before anything is touched — the incumbent stays byte-identical — unless `force=true`.
5. Otherwise: archive the incumbent by rename to `.tapps-mcp/handoffs/archive/<UTC>-<slot|default>.md` (unconditional — conflict or not), prune the archive to the newest 20, write the new markdown to a temp file in the target's own directory, `fsync` it, `os.replace` it onto the target path, then `fsync` the parent directory.

The mode key `handoff_conflict_mode` (`off` / `warn` / `block`, default `warn`) and `handoff_conflict_window_hours` (default 12) live in `.tapps-mcp.yaml`, following the `linear_enforce_cache_gate` precedent. The tool response reports the outcome as `conflict_status` — one of `off` (guard archived silently), `clear` (no foreign incumbent), `unknown` (incumbent's owner unprovable), or `overwritten` (a named different program's recent handoff was replaced) — the only value that degrades the envelope and adds a warning naming the displaced program and its archive path. `list_handoffs()` is the single enumeration site for every live handoff (default plus each slot, excluding `archive/`), consumed by `tapps-continue-session`, CLI `handoff list`, and `fleet_audit`.

Above the guard, `write_handoff()` keeps its pre-slot ordering — file write, git context sync, metadata build, brain mirror, then (when `session_end=true`) `run_session_end()` — but the `slot` now threads all the way through the last step: the session-end search reads back the handoff at that same slot (`load_and_lint_handoff(project_root, slot)`) instead of whatever program owns the default file, so a slotted session's retrieval query is derived from its own handoff.
